...
addcmd('serverlist', {'slist'}, function(args, speaker)
    ...
    if not canOpenServerList then return end
    canOpenServerList = false  -- lock command usage

    local success, listreq = pcall(function()
        return httprequest({Url = string.format("https://games.roblox.com/v1/games/%d/servers/Public?sortOrder=Desc&limit=250&excludeFullGames=false", PlaceId)})
    end)

    if not success or not listreq then
        notify("Serverlist", "Failed to fetch servers")
        canOpenServerList = true
        return
    end
    ...
    if listbody and listbody.data then
        ...
        refreshbutton.MouseButton1Click:Connect(function()
            ...
            local refreshSuccess, refreshReq = pcall(function()
                return httprequest({Url = string.format("https://games.roblox.com/v1/games/%d/servers/Public?sortOrder=Desc&limit=250&excludeFullGames=false", PlaceId)})
            end)

            if refreshSuccess and refreshReq then
                listbody = HttpService:JSONDecode(refreshReq.Body)
                if listbody and listbody.data then
                    addservertemplate()
                else
                    notify("Serverlist", "No server data found on refresh")
                end
            else
                notify("Serverlist", "Failed to refresh server list")
            end
        end)

        FRAME:TweenPosition(UDim2.new(0.5, -225, 0, 100), "InOut", "Quart", 0.5, true)

        task.wait(0.2)

        Exit.MouseButton1Click:Connect(function()
            FRAME:TweenPosition(UDim2.new(0.5, -225, 0, -500), "InOut", "Quart", 0.5, true)
            task.wait(0.2)
            FRAME:Destroy()
            canOpenServerList = true
        end)
        ...
        notify("Serverlist", "No server data found")
        canOpenServerList = true
    end
end)
...
addcmd('ThunderClientLite',{},function(args, speaker)
